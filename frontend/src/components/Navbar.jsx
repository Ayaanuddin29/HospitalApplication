...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-500'>
       <Link to='/'>
       <img  className='w-14 h-14 cursor-pointer' src={assets.logo1}/>
       </Link>
        ...
        <div className='flex items-center gap-4'>
                ...
                token&&userData?<div className='flex items-center cursor-pointer group relative'>
                    ...
                    <div className='absolute top-0 right-0 pt-14 font-medium text-base text-gray-600 z-20 hidden group-hover:block'>
                        <div className='min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4'>
                            ...
                            <p className='hover:text-black-900 cursor-pointer ' onClick={()=>{logout()}}>Logout</p>
                        </div>
                    </div>
                </div>: <button onClick={()=>{
                    navigate('/login')
           ...
           <div className={` ${showMenu?'fixed w-full':'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all `}>
            <div className='flex items-center justify-between px-0'>
                <img className='w-36' onClick={()=>navigate('/')} src={assets.logo1} alt="" />
